...
# hyperparameters (identical to transformer.py except bpe_vocab_size — the one variable)
...
seed = None
bpe_vocab_size = 4096 # THE swapped variable: subword BPE vocab instead of the 65-char vocab
...
# checkpoints live in their own dir so the BPE run never overwrites the char model
CKPT_DIR = os.path.join(ROOT, 'models', 'bpe')
META_PATH = os.path.join(CKPT_DIR, 'meta.pt')

def train_bpe(corpus):
    # byte-level BPE (GPT-2 style): every byte is representable, so no [UNK].
...
# training always builds the tokenizer from the current bpe_vocab_size (vocab IS the
# variable); an imported notebook loads the exact tokenizer the weights trained with.
...
# create a mapping from text to integers (now subword ids, not single chars)
...
# run it all — guarded so `import transformer_bpe` only loads the classes/config above,
# not this training run. notebooks can `from transformer_bpe import GPTLanguageModel`.
if __name__ == '__main__':
    ...
    # save run config + the trained BPE tokenizer once so any snapshot can be rebuilt and
    # decoded without retraining the tokenizer (mirrors the char run's stoi/itos in meta.pt)
    torch.save({
        'config': {
    ...
    for iter in range(max_iters):
        ...
        if iter % eval_interval == 0 or iter == max_iters - 1:
            ...
            if losses['val'] < best_val_loss:
                ...
                print(f"  new best val loss {best_val_loss:.4f} -> saved models/bpe/model.pt")
    ...
    print(f"Best val loss: {best_val_loss:.4f} (weights saved to models/bpe/model.pt)")
